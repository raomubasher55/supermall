...
const FreezingOrders = () => {
    ...
    useEffect(() => {
      const fetchOrders = async () => {
        try {
          setLoader(true)
          const response = await fetch(`${import.meta.env.VITE_API_URL}/api/v1/order/all-order?status=freeze`, {
            method: 'GET',
            headers: {
  ...
  return (
    <>
      ...
      <div className="max-w-4xl mx-auto my-8">
        <h1 className="text-2xl font-bold text-gray-800 mb-6">Freeze Orders</h1>
        {orders.length > 0 ? (
              ...
              <div className="flex justify-between text-sm text-gray-500">
                ...
                <span
                  className={`bg-yellow-400 text-yellow-800 px-2 py-1 rounded-full text-xs font-semibold ${
                    order.status === 'unpaid' ? 'bg-red-200 text-red-800' : 'bg-blue-600 text-white'
                  }`}
                >
                  {order.status}
                </span>
          ...
          <p className="text-lg text-gray-500">No Freeze orders found.</p>
        )}
      </div> 
    </>
  )
